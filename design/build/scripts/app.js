(function() {
  $(document).ready(function() {
    ...
    if (typeof $.scrollTo === 'function') {
      ...
      $(window).scroll(function() {
        if ($(window).scrollTop() > 400) {
          if ($(window).width() >= 640) {
            return $(scrollTopButton).show();
          }
        } else {
          return $(scrollTopButton).hide();
        }
      }).scroll();
    }
    ...
    $('ul.menu > li > a').click(function(e) {
      ...
      e.preventDefault();
      return $('div.page').attr('class', 'page feed').addClass('color-' + $(this).parent().index());
    });
    ...
    if (typeof $.prototype.scrollToFixed === 'function') {
      $('section.notifications').scrollToFixed({
        marginTop: 110
      });
    }
    Alert = {
